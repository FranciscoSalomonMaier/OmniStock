...
let getCompanyId: () => string | null = () =>
  localStorage.getItem("omnistock_company_id");
export function configureApiCompany(getter: () => string | null) {
  getCompanyId = getter;
}
...
export async function apiRequest<T>(
  ...
  if (requestVersion !== sessionVersion)
    throw new ApiError("Sessao encerrada", 401);
  if (response.status === 401 && !options.skipAuthRefresh && getAccessToken()) {
    refreshPromise ??= renewSession().finally(() => {
      refreshPromise = null;
    });
    if ((await refreshPromise) && requestVersion === sessionVersion)
      return parseResponse<T>(await send(path, options));
    onSessionExpired();
  }
