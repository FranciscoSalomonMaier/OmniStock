...
import type { ChannelConnection, SalesChannel } from "../../types/integration";
export function IntegrationsPage() {
  const { activeCompany, activeMembership } = useCompany(),
    [catalog, setCatalog] = useState<SalesChannel[]>([]),
  ...
  async function connect(channel: SalesChannel) {
    ...
    if (!name) return;
    try {
      await integrationService.create(channel.id, name);
      await load();
    } catch (e) {
      setError(e instanceof Error ? e.message : "Falha ao criar conexão");
    }
  }
  return (
    <main className="page">
      <div className="page-title">
        <div>
          <h1>Integrações</h1>
          <p className="muted">Canais de venda da empresa ativa.</p>
        </div>
      </div>
      ...
      <div className="cards">
        {catalog.map((channel) => (
          <section className="company-card" key={channel.id}>
            <strong>{channel.name}</strong>
            <span>{channel.type}</span>
            <p>{channel.description}</p>
            {canManage && (
              <button onClick={() => void connect(channel)}>
                Preparar conexão
              </button>
            )}
          </section>
