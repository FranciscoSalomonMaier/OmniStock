...
export function RegisterPage() {
  const { register, isAuthenticated } = useAuth(); const [form, setForm] = useState({ name: '', email: '', password: '', confirmation: '' })
  ...
  async function submit(event: FormEvent) {
    ...
    setSubmitting(true)
    try { const result = await register({ name: form.name, email: form.email, password: form.password }); navigate('/verification-pending', { replace: true, state: { email: result.email } }) }
    catch (reason) { setError(reason instanceof Error ? reason.message : 'Não foi possível cadastrar') } finally { setSubmitting(false) }
  }
  const field = (key: keyof typeof form) => (event: ChangeEvent<HTMLInputElement>) => setForm({ ...form, [key]: event.target.value })
  return <><h1>Criar conta</h1><p className="subtitle">Comece com acesso de visualização.</p><form onSubmit={submit} className="auth-form">
    ...
    {error && <p className="form-error" role="alert">{error}</p>}<button type="submit" disabled={submitting}>{submitting ? 'Criando conta...' : 'Criar conta'}</button>
